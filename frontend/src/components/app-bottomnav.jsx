...
function BottomNav({handleCreate}) {
    ...
    return (
        <>
            {
                selected ? (
                    <nav className={`fixed bottom-0 left-0 right-0 z-50 bg-black text-white border-t border-gray-700 ${selected === "" ? "animate-slide-down" : "animate-slide-up"}`}>
                    ...
                    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-black text-white border-t border-gray-700">
                        <div className="flex items-center justify-center mx-auto py-2 border-t border-gray-700 py-4">
                            {navItems.map((item, index) => (
                                item.type === "link" ? (
                                    ...
                                    <Link key={index} onClick={() => setSelected(selected === item.name ? "" : item.name)} className="flex flex-col items-center justify-center mx-4">
                                        <item.icon className="w-6 h-6" />
                                    </Link>
                                )
                            ))}
