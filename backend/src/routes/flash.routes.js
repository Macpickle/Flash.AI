...
router.post("/:docId", auth, async (req, res) => {
  try {
    ...
    if (
      flash.type === "multiple_choice" &&
      (!flash.multipleChoice ||
        !flash.multipleChoice.options ||
    ...
    if (
      flash.type === "matching" &&
      (!flash.matching ||
        !flash.matching.pairs ||
        flash.matching.pairs.length < 2)
    ) {
      return res
        .status(400)
        .json({ message: "Matching cards must have at least two pairs" });
    }
    ...
    res
      .status(400)
      .json({ message: "Could not create flash card", error: error.message });
  }
});
...
router.get("/doc/:docId", auth, async (req, res) => {
  try {
    ...
    const flashCards = await Flash.find({ docId: doc._id }).sort({
      createdAt: -1,
    });
    res.json(flashCards);
  } catch (error) {
    res
      .status(500)
      .json({ message: "Could not fetch flash cards", error: error.message });
  }
});
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Could not fetch flash card", error: error.message });
  }
});
...
router.patch("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Could not update flash card", error: error.message });
  }
});
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Could not delete flash card", error: error.message });
  }
});
