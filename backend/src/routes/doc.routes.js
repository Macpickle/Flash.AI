...
router.post("/", auth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Could not create doc", error: error.message });
  }
});
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Could not fetch docs", error: error.message });
  }
});
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Could not fetch doc", error: error.message });
  }
});
...
router.patch("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Could not update doc", error: error.message });
  }
});
...
router.delete("/:id", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Could not delete doc", error: error.message });
  }
});
